...
import {
  FREE_LAUNCH_RESEARCH_ESTIMATE_MS,
  isFreeLaunchResearchFailed,
...
export default function LaunchProgress({ launch }: { launch: LaunchState }) {
  ...
  const failed = isFreeLaunchResearchFailed(launch);
  ...
  const isFreeResearch =
    !isCampaignBuild && launch.project.phase === 'researching' && !failed;
  ...
  const failureDetail =
    launch.researchProgress.find((step) => step.status === 'error')?.detail ||
    (isZh ? '产品分析失败' : 'Product analysis failed');

  const handleRetry = async () => {
    if (retrying) return;
    setRetrying(true);
    const next = resetFreeLaunchResearch(launch, isZh);
        ...
        <section className="rounded-3xl border border-white/10 bg-white/[0.035] p-6 sm:p-8">
          <p
            className={`text-[10px] font-semibold uppercase tracking-[0.24em] ${
              failed ? 'text-red-300' : 'text-brand-300'
            }`}
          >
            {failed
              ? isZh
          ...
          {failed ? (
            <div className="mt-6 rounded-2xl border border-red-400/25 bg-red-400/[0.08] px-4 py-4">
              <p className="text-sm leading-6 text-red-100">{failureDetail}</p>
              <p className="mt-2 text-xs leading-5 text-red-200/70">
                {isZh
                  ? '成功前不会生成项目文档，也不会显示「已准备好」。'
                  : 'We won’t create a project document or show “ready” until research succeeds.'}
              </p>
              <button
                type="button"
                onClick={() => void handleRetry()}
                disabled={retrying}
                className="mt-4 rounded-full bg-white px-5 py-2.5 text-xs font-bold text-black transition hover:bg-zinc-200 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {retrying
                  ? isZh
                    ? '正在重试…'
                  ...
                  {step.detail && (
                    <p
                      className={`mt-1 text-xs leading-5 ${
                        step.status === 'error' ? 'text-red-300/70' : 'text-zinc-600'
                      }`}
                    >
                      {step.detail}
                    </p>
          ...
          <p className="mt-6 text-xs leading-5 text-zinc-600">
            {failed
              ? isZh
                ? '重试会重新读取产品网站并生成项目文档。'
                : 'Retry will re-read the product site and generate the project document.'
              : isFreeResearch
                ? isZh
