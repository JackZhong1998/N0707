  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-7 sm:px-8 sm:py-10">
      <header className="border-b border-white/[0.08] pb-7">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-brand-300">{isZh ? '30 天冷启动' : '30-Day Campaign'} · v{blueprint.revision}</p><h1 className="mt-2 max-w-3xl font-[family-name:var(--font-display)] text-3xl font-black tracking-tight text-white sm:text-4xl">{isZh ? '推广蓝图' : 'Launch Blueprint'}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-500">{isZh ? '所有渠道都沿着这套主线推进，再根据各平台的语境分别执行。' : 'The shared campaign spine. Each channel only translates it into native execution.'}</p></div><Link href="/app/brief" className="rounded-full border border-white/10 px-4 py-2 text-xs font-semibold text-zinc-300 hover:bg-white/[0.06]">← {isZh ? '冷启动简报' : 'Launch Brief'}</Link></div>
      </header>
