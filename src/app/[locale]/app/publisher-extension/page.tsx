      ...
      <div className="mx-auto max-w-4xl pb-16">
        <section className="overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.025]">
          <div className="p-6 sm:p-9">
            ...
            {needsUpgrade && (
              <div className="mt-7 overflow-hidden rounded-2xl border border-amber-300/20 bg-amber-300/[0.05]">
                <div className="border-b border-amber-300/15 px-5 py-4">
                ...
                {releaseNotes && (
                  <ul className="space-y-2 px-5 py-4 text-xs leading-relaxed text-amber-100/80">
              ...
              <div className="mt-7 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <Link
                      href={returnTo}
                      className="mt-3 inline-flex h-10 items-center rounded-full bg-white px-5 text-sm font-semibold text-black transition-colors hover:bg-zinc-200"
                    >
                      {returningToTask
                        ? isZh
                          ? '返回当前任务并继续发布'
              ...
              <div className={needsUpgrade ? 'mt-5' : 'mt-7'}>
                <div className="flex flex-wrap items-center gap-3">
                  <a
                    href={downloadUrl}
                    download
                    className="inline-flex h-11 items-center gap-2 rounded-full bg-white px-6 text-sm font-semibold text-black transition-colors hover:bg-zinc-200"
                  >
                    <DownloadIcon />
                    {needsUpgrade
                      ? isZh
          ...
          <div className="border-t border-white/[0.06] p-6 sm:p-9">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {steps.map(([title, description], index) => (
                <div key={title} className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
                  <div className="flex items-start gap-3">
          ...
          <div className="border-t border-white/[0.06] p-6 sm:p-9">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {CHANNEL_CAPABILITIES.filter(
                ...
                <div key={item.channelId} className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {capabilityLabels(item, isZh).map((label) => (
                      <span
                        key={label}
                        className="rounded-full border border-white/10 px-2 py-1 text-[10px] text-zinc-500"
                      >
                        {label}
                      </span>
                    ))}
          ...
          <div className="border-t border-white/[0.06] p-6 sm:p-9">
            ...
            <div className="mt-4 grid gap-3 text-xs leading-relaxed text-zinc-500 sm:grid-cols-3">
              <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
                <p className="font-semibold text-white">{isZh ? '按平台独立适配' : 'Platform-specific adapters'}</p>
                <p className="mt-1">
              ...
              <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
                <p className="font-semibold text-white">{isZh ? '不保存登录信息' : 'No login storage'}</p>
                <p className="mt-1">
                  {isZh ? '直接使用你浏览器中已有的平台登录状态。' : 'Uses the sessions already present in your browser.'}
                </p>
              </div>
              <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
                <p className="font-semibold text-white">{isZh ? '不自动点击发布' : 'You confirm publishing'}</p>
                <p className="mt-1">
