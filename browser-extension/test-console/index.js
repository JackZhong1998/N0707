...
function directorySample() {
  return {
    ...
    longDescription: `NowBuild is a launch workflow assistant built for solo founders and one-person companies. It turns product information into a practical thirty-day distribution plan and helps founders prepare content for the channels where their audiences already spend time. Instead of opening every website, rewriting the same product story, and tracking submissions in scattered notes, a founder can keep one launch kit and reuse it across the launch process.

The product is designed around a simple problem. Most small teams can build a useful product, but they do not have the time or specialist knowledge required to distribute it consistently. NowBuild organizes the product name, website, short description, long description, logo, screenshots, categories, pricing, and social links in one place. It then adapts those facts for social networks, article platforms, launch communities, and software directories while keeping the founder in control of the final publishing decision.

A local browser assistant performs the repetitive execution work inside the founder's existing signed-in browser. It can open a supported platform, recognize whether login is required, use the existing Google session when the site supports it, fill the correct fields, upload prepared assets, move through multi-step forms, and stop before the irreversible final submission. This approach avoids asking users to share account passwords and makes the workflow compatible with platforms that do not provide a publishing API.

NowBuild also records useful outcomes after launch. For supported social and content channels, it can collect public metrics such as views, reactions, comments, and links to the published item. For directory submissions, it keeps a clear status trail showing which site was opened, which fields were prepared, whether login or another manual action was required, and whether the listing was saved, submitted, approved, or rejected.

The main use case is a founder who has recently shipped a web app, mobile app, browser extension, or small software service and wants to create initial awareness without buying ads or hiring a marketing team. The founder prepares one complete launch kit, reviews channel-specific content, and lets the assistant handle repetitive browser work. NowBuild is especially useful for indie hackers, bootstrapped founders, makers, and tiny teams that need repeatable execution more than another blank writing tool.

The goal is to make a month of launch work concrete and executable: consistent publishing, broad directory coverage, reusable product assets, and a reliable record of what happened on every channel. By reducing setup work and context switching, NowBuild helps founders spend more time talking to users, improving the product, and learning which distribution channels create meaningful attention.`,
    categories: ['Artificial Intelligence', 'Marketing Tools', 'Productivity'],
...
async function bundledImageAsset(path, kind, name) {
  const response = await fetch(chrome.runtime.getURL(path));
  if (!response.ok) throw new Error(`无法读取内置图片：${name}`);
  const blob = await response.blob();
  const dataUrl = await new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = () => resolve(reader.result);
    reader.onerror = () => reject(new Error(`无法读取内置图片：${name}`));
...
async function useBundledNowBuildLogo() {
  try {
    const [logo, screenshot] = await Promise.all([
      bundledImageAsset('assets/nowbuild-logo.png', 'logo', 'nowbuild-logo.png'),
      bundledImageAsset(
        'assets/nowbuild-home-preview.png',
        'screenshot',
        'nowbuild-home-preview.png'
      ),
    ]);
    state.directoryAssets = [
      ...state.directoryAssets.filter(
        (asset) => asset.kind !== 'logo' && asset.kind !== 'screenshot'
      ),
      logo,
      screenshot,
    ];
    renderAssetSummary();
    addLog('asset_ready', '已载入 NowBuild Logo 和横版产品截图，可测试目录素材上传');
  } catch (error) {
    addLog('asset_failed', error.message);
  }
}
...
async function copyDirectoryFlows() {
  const response = await sendCommand('GET_DIRECTORY_FLOWS');
  await navigator.clipboard.writeText(JSON.stringify(response.flows || {}, null, 2));
  addLog('flows_copied', '已复制全部录制流程 JSON，可保存到源码或导入其他浏览器');
}

async function importDirectoryFlows() {
  const raw = window.prompt('粘贴 NowBuild 录制流程 JSON');
  if (!raw) return;
  const flows = JSON.parse(raw);
  const response = await sendCommand('IMPORT_DIRECTORY_FLOWS', { flows });
  addLog('flows_imported', `已导入 ${Object.keys(response.flows || {}).length} 个站点流程`);
  await refreshDirectoryFlowStatus();
}
...
document.getElementById('copy-directory-flows').addEventListener('click', () => {
  void copyDirectoryFlows().catch((error) => addLog('failed', error.message));
});
document.getElementById('import-directory-flows').addEventListener('click', () => {
  void importDirectoryFlows().catch((error) => addLog('failed', error.message));
});
