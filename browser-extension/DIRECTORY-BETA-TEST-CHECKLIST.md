# NowBuild 0.9.14 · 目录提交与流程录制 Beta 测试

## 测试前

1. 在 `chrome://extensions` 重新加载插件，确认版本为 `0.9.14`，图标为黑底白绿 NowBuild 标志。
...
1. 在测试台选中目标目录并点击“录制这个站点”。
2. 如需登录，由测试者自行点击登录并完成账号验证；插件不得记录密码或账号信息。
3. 按真实顺序填写或点击，每完成一页可点击悬浮面板的 “Save stage”。
4. 最后一页不要点击平台的最终提交按钮，点击悬浮面板的 “Finish”。
5. 回到测试台确认显示已学习的页面数和动作数。
6. 再执行一次目录 Dry Run，确认各页按录制顺序预填，并在最终提交前停止。
7. 确认内置 `nowbuild-logo.png` 能进入真实 Logo 文件上传控件。

## 0.9.14 不会做的动作

- 未经用户在 NowBuild 明确勾选授权时，不自动点击最终提交。
- 仅 3 个免费、无已知阻塞的白名单目录允许自动最终提交；未识别成功回执时立即要求用户检查。
- 不自动登录 Google/GitHub。
