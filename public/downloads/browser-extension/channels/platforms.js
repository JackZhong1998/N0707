...
const PLATFORM_ADAPTER_VERSION = 'multi-0.5.0';
...
function loginWall(platformName) {
  ...
  if (
    ...
    throw new Error('请先完成 Indie Hackers 账号与 Profile 初始化，再重新运行 Dry Run');
  }
  const patterns = [
  ...
  if (patterns.some((pattern) => pattern.test(text))) {
    throw new Error(`请先在 ${platformName} 网页端完成登录，再重新运行 Dry Run`);
  }
}
...
async function fillReddit(request) {
  ...
  try {
    ...
    if (chooseCommunity) {
      throw new Error('请先在 Reddit 页面选择一个允许发帖的社区，然后重新运行 Dry Run');
    }
    throw error;
  }
...
function isEditorUrl(channel, urlString) {
  ...
  if (channel === 'indie_hackers') return /\/post\/?$/.test(url.pathname);
  return false;
}
...
function watchForPublish() {
  ...
  const timer = window.setInterval(() => {
    ...
    if (isEditorUrl(CHANNEL, window.location.href)) return;
    window.clearInterval(timer);
    emit('published', {
      message: '平台已离开编辑页，请在测试报告中确认帖子地址',
...
function collectHackerNewsMetrics() {
  ...
  return {
    points: runtime.parseCount(score?.textContent),
...
function collectPlatformMetrics() {
  ...
  const selectorMaps = {
    ...
    hashnode: {
      reactions: ['[aria-label*="reaction" i]', '[data-testid*="reaction" i]'],
      comments: ['[aria-label*="comment" i]', '[data-testid*="comment" i]'],
      views: ['[aria-label*="view" i]', '[data-testid*="view" i]'],
    },
    indie_hackers: {
      votes: ['[aria-label*="vote" i]', '[class*="vote-count" i]'],
      comments: ['[aria-label*="comment" i]', '[class*="comment-count" i]'],
    },
  };
  return metricsFromSelectors(selectorMaps[CHANNEL] || {});
}
