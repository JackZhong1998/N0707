# NowBuild 发布助手 0.5.0 Beta 测试清单

## 测试前

1. 在 `chrome://extensions` 重新加载插件，确认版本显示为 `0.5.0`。
...
| 6 | Hashnode | 登录并至少创建一个 Publication | 标题和正文被填入；允许 Hashnode 自动保存草稿，不发布 |
| 7 | Indie Hackers | 登录 Indie Hackers | 帖子标题和正文被填入，停在最终发布前 |
...
Medium 自定义域名和 Hashnode 自定义域名暂不纳入 0.5.0；先用平台官方域名测试。
