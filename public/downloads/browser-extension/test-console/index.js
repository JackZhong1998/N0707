...
const TERMINAL_STATUSES = new Set(['prepared', 'published', 'collected', 'failed', 'cancelled']);
...
function updatePublishForm() {
  ...
  elements.urlHelp.textContent =
    channel === 'hacker_news' ? '填写后创建 link submission' : '可选；会追加到正文';
  ...
  if (isLive) {
    ...
    elements.platformNotice.textContent = '填写产品 URL 时按 HN 的 link submission 执行，只填写标题和 URL；不填写 URL 时才会创建 text submission。';
    ...
    elements.platformNotice.textContent = 'Dry Run 会填写 Hashnode 文章标题与正文并停在编辑器。平台可能自动保留草稿。';
  } else if (channel === 'indie_hackers') {
    elements.platformNotice.textContent = 'Dry Run 会填写 Indie Hackers 帖子标题与正文并停在发布前。';
  } else {
...
elements.directoryId.addEventListener('change', updateDirectoryNotice);
...
elements.metricsChannel.addEventListener('change', () => {
  const placeholders = {
    ...
    hashnode: 'https://hashnode.com/post/...',
